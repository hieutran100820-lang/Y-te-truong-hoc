...
const SchoolsPage: React.FC<SchoolsPageProps> = ({ selectedYear, dynamicFields, currentUser, schools, setSchools, healthRecords, setHealthRecords, showNotification }) => {
    ...
    return (
        <div className="animate-fade-in">
            <h2 className="text-3xl font-bold text-slate-800 mb-6">Quản lý Trường học</h2>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-xl font-semibold text-slate-800">Danh sách trường</h3>
                         {currentUser.role === 'admin' && (
                            <button 
                                onClick={() => setAddModalOpen(true)}
                                className="bg-brand hover:bg-brand-dark text-white font-bold py-2 px-3 rounded text-sm transition-colors duration-300"
                             >
                               Thêm mới
                            </button>
                         )}
                     ...
                     <div className="mb-4 relative">
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-md focus:ring-brand focus:border-brand sm:text-sm"
                        />
                    ...
                    <ul className="space-y-2 max-h-[55vh] overflow-y-auto">
                        {filteredSchools.map(school => (
                            <li 
                                ...
                                className={`group p-3 rounded-md cursor-pointer transition-all duration-200 border-l-4 flex justify-between items-center ${selectedSchool?.id === school.id 
                                    ...
                                    : 'hover:bg-slate-100 hover:border-slate-300 border-transparent'
                                }`}
                            >
                                <div className="flex-grow">
                                    <p className="font-semibold text-slate-800">{school.name}</p>
                                    <p className="text-sm text-slate-500">{school.level} - {school.location}</p>
                                </div>
                                {currentUser.role === 'admin' && (
                                    <button
                                        onClick={(e) => {
                        ...
                        {filteredSchools.length === 0 && (
                            <li className="text-center text-slate-500 py-4">Không tìm thấy trường nào.</li>
                        )}
                    </ul>
                </div>
                <div className="lg:col-span-2">
                    {selectedSchool ? (
                        ...
                        <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm text-center h-full flex flex-col justify-center">
            ...
            <Modal isOpen={isAddModalOpen} onClose={() => setAddModalOpen(false)} title="Thêm trường học mới">
                <form onSubmit={handleAddSchool} className="space-y-4">
                    <div>
                        <label htmlFor="name" className="block text-sm font-medium text-slate-700">Tên trường</label>
                        <input
                            type="text"
                            id="name"
                            name="name"
                            ...
                            required
                            className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-brand focus:border-brand sm:text-sm"
                        />
                    </div>
                     <div>
                        <label htmlFor="level" className="block text-sm font-medium text-slate-700">Phân cấp</label>
                        <select
                            id="level"
                            name="level"
                            value={newSchool.level}
                            onChange={handleInputChange}
                            className="mt-1 block w-full pl-3 pr-10 py-2 bg-white border border-slate-300 rounded-md shadow-sm text-slate-900 focus:outline-none focus:ring-brand focus:border-brand sm:text-sm"
                        >
                     ...
                     <div>
                        <label htmlFor="location" className="block text-sm font-medium text-slate-700">Địa điểm</label>
                        <input
                            type="text"
                            id="location"
                            name="location"
                    ...
                    <div className="flex justify-end pt-2">
                        <button type="button" onClick={() => setAddModalOpen(false)} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded mr-2 hover:bg-slate-300 transition-colors duration-300">
                            Hủy
                        </button>
            ...
            <Modal isOpen={!!schoolToDelete} onClose={() => setSchoolToDelete(null)} title="Xác nhận xóa trường">
                <div className="py-2">
                    <p className="text-slate-700">Bạn có chắc chắn muốn xóa trường <span className="font-bold">{schoolToDelete?.name}</span> không?</p>
