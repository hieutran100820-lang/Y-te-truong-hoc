...
const SettingsPage: React.FC<SettingsPageProps> = ({ schoolYears, setSchoolYears, dynamicFields, setDynamicFields, healthRecords, setHealthRecords, seedDatabase, showNotification }) => {
    ...
    return (
        <div className="animate-fade-in space-y-6">
            <h2 className="text-3xl font-bold text-slate-800">Cấu hình Hệ thống</h2>

            {/* School Year Management */}
            <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center border-b border-slate-200 pb-2 mb-4">
                    <h3 className="text-xl font-semibold text-slate-800">Quản lý Năm học</h3>
                    <button onClick={handleOpenAddYearModal} className="bg-brand hover:bg-brand-dark text-white font-bold py-2 px-3 rounded text-sm transition-colors duration-300">
                        Tạo năm học mới
                    </button>
                </div>
                <div className="space-y-3">
                    {[...schoolYears].sort((a,b) => a.year.localeCompare(b.year)).map(sy => (
                        <div key={sy.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-md hover:bg-slate-100">
                            <div>
                                <span className="font-semibold text-slate-800">{sy.year}</span>
                                {sy.isCurrent && <span className="ml-2 text-xs font-bold text-white bg-green-500 px-2 py-1 rounded-full">Hiện tại</span>}
                                {sy.isLocked && <span className="ml-2 text-xs font-bold text-white bg-slate-500 px-2 py-1 rounded-full">Đã khóa</span>}
                            </div>
            ...
            <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center border-b border-slate-200 pb-2 mb-4">
                    <h3 className="text-xl font-semibold text-slate-800">Quản lý Cấu hình Động (Nâng cao)</h3>
                    <button onClick={() => handleOpenFieldModal()} className="bg-brand hover:bg-brand-dark text-white font-bold py-2 px-3 rounded text-sm transition-colors duration-300">
                        Thêm trường thông tin mới
                    </button>
                </div>
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end mb-4">
                     <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Chọn Tab để sửa</label>
                        <select 
                ...
                {filteredFields.length > 0 ? (
                    <div className="space-y-3 mt-4 border-t border-slate-200 pt-4">
                        {filteredFields.map(field => (
                            <div key={field.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-md">
                                <div>
                                    <span className="font-semibold text-slate-800">{field.label}</span>
                                    <span className="ml-3 text-xs text-slate-500 bg-slate-200 px-2 py-1 rounded-full">{field.type}</span>
                                </div>
                                <div className="space-x-3">
                    ...
                    <div className="mt-6 text-center text-slate-500 py-10 border-t border-slate-200">
            ...
            <Modal isOpen={isAddYearModalOpen} onClose={() => setIsAddYearModalOpen(false)} title="Tạo năm học mới">
                <form onSubmit={handleAddYear} className="space-y-4">
                    <div>
                        <label htmlFor="newYearName" className="block text-sm font-medium text-slate-700">Tên năm học (VD: 2025-2026)</label>
                    ...
                    <div className="flex justify-end pt-2 space-x-2">
                        <button type="button" onClick={() => setIsAddYearModalOpen(false)} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded hover:bg-slate-300 transition-colors duration-300">Hủy</button>
                        <button type="submit" className="bg-brand hover:bg-brand-dark text-white font-bold py-2 px-4 rounded transition-colors duration-300">Tạo</button>
                    </div>
                </form>
            </Modal>
            <Modal isOpen={!!yearToEdit} onClose={() => setYearToEdit(null)} title={`Chỉnh sửa năm học: ${yearToEdit?.year}`}>
                <form onSubmit={handleEditYear} className="space-y-4">
                     <div>
                        <label htmlFor="editedYearName" className="block text-sm font-medium text-slate-700">Tên năm học (VD: 2025-2026)</label>
                    ...
                    <div className="flex justify-end pt-2 space-x-2">
                        <button type="button" onClick={() => setYearToEdit(null)} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded hover:bg-slate-300 transition-colors duration-300">Hủy</button>
            ...
            <Modal isOpen={!!yearToLock} onClose={() => setYearToLock(null)} title="Xác nhận khóa năm học">
                <div className="py-2"><p className="text-slate-700">Bạn có chắc chắn muốn khóa năm học <span className="font-bold">{yearToLock?.year}</span> không?</p><p className="text-sm text-orange-600 mt-2">Sau khi khóa, bạn sẽ không thể chỉnh sửa dữ liệu của năm học này nữa.</p></div>
                <div className="flex justify-end pt-4 space-x-2"><button type="button" onClick={() => setYearToLock(null)} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded hover:bg-slate-300 transition-colors duration-300">Hủy</button><button type="button" onClick={handleConfirmLock} className="bg-orange-600 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded transition-colors duration-300">Xác nhận Khóa</button></div>
            </Modal>
            <Modal isOpen={!!yearToUnlock} onClose={() => setYearToUnlock(null)} title="Xác nhận mở khóa năm học">
                <div className="py-2"><p className="text-slate-700">Bạn có chắc chắn muốn mở khóa năm học <span className="font-bold">{yearToUnlock?.year}</span>?</p><p className="text-sm text-yellow-600 mt-2">Mở khóa sẽ cho phép chỉnh sửa lại toàn bộ dữ liệu của năm học này.</p></div>
                <div className="flex justify-end pt-4 space-x-2"><button type="button" onClick={() => setYearToUnlock(null)} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded hover:bg-slate-300 transition-colors duration-300">Hủy</button><button type="button" onClick={handleConfirmUnlock} className="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded transition-colors duration-300">Xác nhận Mở khóa</button></div>
            </Modal>
            <Modal isOpen={!!yearToDelete} onClose={() => setYearToDelete(null)} title="Xác nhận xóa năm học">
                <div className="py-2"><p className="text-slate-700">Bạn có chắc chắn muốn xóa vĩnh viễn năm học <span className="font-bold">{yearToDelete?.year}</span>?</p><p className="text-sm text-red-600 mt-2">Toàn bộ dữ liệu liên quan đến năm học này sẽ bị xóa. Hành động này không thể hoàn tác.</p></div>
                <div className="flex justify-end pt-4 space-x-2"><button type="button" onClick={() => setYearToDelete(null)} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded hover:bg-slate-300 transition-colors duration-300">Hủy</button><button type="button" onClick={handleConfirmDeleteYear} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded transition-colors duration-300">Xác nhận Xóa</button></div>
            </Modal>
            ...
            <Modal isOpen={isFieldModalOpen} onClose={() => setIsFieldModalOpen(false)} title={fieldData.id ? 'Chỉnh sửa trường thông tin' : 'Thêm trường thông tin mới'}>
                <form onSubmit={handleSaveField} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Nhãn hiển thị</label>
                        <input type="text" name="label" value={fieldData.label} onChange={handleFieldInputChange} required className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-brand focus:border-brand sm:text-sm" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Kiểu dữ liệu</label>
                        <select name="type" value={fieldData.type} onChange={handleFieldInputChange} className="mt-1 block w-full pl-3 pr-10 py-2 bg-white border border-slate-300 rounded-md shadow-sm text-slate-900 focus:outline-none focus:ring-brand focus:border-brand sm:text-sm">
                    ...
                    {(fieldData.type === 'select' || fieldData.type === 'droplist') && (
                        <div>
                            <label className="block text-sm font-medium text-slate-700">Tùy chọn (phân cách bằng dấu phẩy)</label>
                            <input type="text" name="options" value={fieldData.options} onChange={handleFieldInputChange} className="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-brand focus:border-brand sm:text-sm" />
                        </div>
                    )}
                    <div className="flex justify-end pt-2 space-x-2">
                        <button type="button" onClick={() => setIsFieldModalOpen(false)} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded hover:bg-slate-300 transition-colors duration-300">Hủy</button>
            ...
            <Modal isOpen={!!fieldToDelete} onClose={() => setFieldToDelete(null)} title="Xác nhận xóa trường thông tin">
                 <div className="py-2"><p className="text-slate-700">Bạn có chắc chắn muốn xóa trường <span className="font-bold">{fieldToDelete?.label}</span>?</p><p className="text-sm text-red-600 mt-2">Dữ liệu đã nhập cho trường này sẽ không còn hiển thị. Hành động này không thể hoàn tác.</p></div>
                 <div className="flex justify-end pt-4 space-x-2"><button type="button" onClick={() => setFieldToDelete(null)} className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded hover:bg-slate-300 transition-colors duration-300">Hủy</button><button type="button" onClick={handleConfirmDeleteField} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded transition-colors duration-300">Xác nhận Xóa</button></div>
            </Modal>
            
            {/* Reset Data Modal */}
            <Modal isOpen={isResetModalOpen} onClose={() => setIsResetModalOpen(false)} title="Xác nhận Reset Dữ liệu">
                <div className="py-2">
                    <p className="text-slate-700 text-lg">Bạn có thực sự chắc chắn muốn reset toàn bộ dữ liệu không?</p>
                ...
                <div className="flex justify-end pt-4 space-x-2">
                    <button 
                        ...
                        className="bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded hover:bg-slate-300 transition-colors duration-300"
                    >
                        Hủy
                    </button>
                    <button 
